...
{
    // Java program of above implementation public class GFG {
    ...
    static Pair getMinMax(int arr[],int n)
    {
        ...
        {
            minmax.min = arr[0]; minmax.max = arr[0];
            i = 1;
            /* set the starting index for loop */
        }
            /* In the while loop,pick elements in pair and
            compare the pair with max and min so far */ while (i < n - 1) { // for(; i<n-1;i+=2)
            if (arr[i] > arr[i + 1])
            {
                if (arr[i] > minmax.max)
                {
